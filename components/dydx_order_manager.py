...
class DydxOrderManager:
    ...
    def market_volatility(self, symbol):
        ...
        price_data = self.firebase_db_obj.get_ema_data(collection_name="price_data", id=symbol)

        # TODO :  have to change the start time in both if and else condition's
        ...
        if price_data is None:
            ...
            prices = self.binance_client_obj.price_data_per_interval(symbol=symbol, start_time=str(start_time.timestamp() * 1000),
            ...
            prices = self.binance_client_obj.price_data_per_interval(symbol=symbol, end_time=str(end_time.timestamp() * 1000),
        ...
        self.firebase_db_obj.store_ema_data("price_data", symbol, {"prices": price_data})
        self.firebase_db_obj.store_ema_data("ema_data", symbol, {"ema": volatility_data})
